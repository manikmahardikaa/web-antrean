...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  try {
    const row = await prisma.beritaKesehatan.findUnique({ where: { id_berita: params.id } });
    if (!row) return NextResponse.json({ message: 'Berita tidak ditemukan' }, { status: 404 });
 ...
 *  - JSON: { judul?, deskripsi?, tanggal_penerbitan?, foto_url?(string|null) }
 *  - multipart: fields judul?, deskripsi?, tanggal_penerbitan?, file?(image)
...
export const PUT = withRole('ADMIN')(async (req: Request, { params }: { params: { id: string } }) => {
  try {
    const existing = await prisma.beritaKesehatan.findUnique({ where: { id_berita: params.id } });
    if (!existing) return NextResponse.json({ message: 'Berita tidak ditemukan' }, { status: 404 });
    ...
    if (ct.includes('multipart/form-data')) {
      ...
      if (file && file.size > 0) {
        ...
        const path = `${existing.id_berita}/image_${Date.now()}.${ext}`;
        ...
        const oldPath = extractPathFromPublicUrl(existing.foto_url);
      ...
      if (body?.foto_url !== undefined) {
        ...
        const newUrl = body.foto_url === null ? null : String(body.foto_url);
        if (newUrl === null) {
...
export const DELETE = withRole('ADMIN')(async (_req: Request, { params }: { params: { id: string } }) => {
  try {
    ...
    await prisma.beritaKesehatan.delete({ where: { id_berita: params.id } });
